...
try:
    from backend01 import Stavebnik, Urednik, Zadost, StavebniUrad, Osoba
except ImportError:
...
"""
------------------------------------------------------------
GENEROVÁNÍ ID ZÁVODNÍKA — BLOK JE ZAKOMENTOVÁN
------------------------------------------------------------

Tato funkce vytvoří ID závodníka podle pravidla:
  3 písmena jména + 3 písmena příjmení + poslední 2 čísla roku narození

Pokud ID již existuje, přidává se číselná koncovka:
AdaNov16   →  AdaNov162  →  AdaNov163  →  AdaNov164  …

Příklad:
  Adam Novák 2016  →  AdaNov16
  další Adam Novák 2016  →  AdaNov162

Jak funkci používat:
  existujici = ["AdaNov16", "AdaNov162"]
  nove = vygeneruj_id_zavodnika("Adam", "Novák", 2016, existujici)
...
def odeber_diakritiku(text):
    \"\"\"Pomocná funkce — odstraní diakritiku z jména/příjmení.\"\"\"
    return ''.join(
        c for c in unicodedata.normalize("NFD", text)
        if unicodedata.category(c) != "Mn"
    )

def vygeneruj_id_zavodnika(jmeno, prijmeni, rok_nar, existujici_id):
    \"\"\"
    Vytvoří ID závodníka podle pravidel:
      - 3 písmena jména
      - 3 písmena příjmení
      - 2 čísla roku narození
      - řeší kolize přidáním číselné koncovky
    \"\"\"

    # odebrání diakritiky a ořez mezer
    j = odeber_diakritiku(jmeno).strip().capitalize()
    p = odeber_diakritiku(prijmeni).strip().capitalize()

    # základ ID
    zaklad = j[:3] + p[:3] + str(rok_nar)[-2:]

    # není-li kolize, hotovo
    if zaklad not in existujici_id:
        return zaklad

    # jinak přidáváme číselné koncovky 2,3,4...
    i = 2
    while f\"{zaklad}{i}\" in existujici_id:
        i += 1

    return f\"{zaklad}{i}\"

------------------------------------------------------------
POZDĚJI AKTIVUJEŠ TAK, ŽE SMAŽEŠ TYTO TROJITÉ UVOZOVKY
------------------------------------------------------------
"""